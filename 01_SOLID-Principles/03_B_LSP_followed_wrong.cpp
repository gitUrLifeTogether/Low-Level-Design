    ...
    void processTransactions(){
        for(Account* acc : accounts){
            ...
            if(typeid(*acc)==typeid(fixedDepositAcc)){
                cout<<"skipping widrawal for fixed term account"<<endl;
            }
            else{
                try{
                acc->withdraw(500);
...
/* this is a wrong solution cause then client becomes tightly coupled with other classes
so if we have to do any changes in future , it will be complicated 
also this vioalted the OCP */